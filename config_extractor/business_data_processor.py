...
def create_business_data_files(data, dir_path, item, env, check):
    # Use the '/' operator for path manipulation
    dir_path_original = Path(dir_path) / f"original_{env}_{check}"
    output_file_original = dir_path_original / f"data_{env}_{check}-{item}-original.json"

    dir_path_processed = Path(dir_path) / f"processed_{env}_{check}"
    output_file_processed = dir_path_processed / f"data_{env}_{check}-{item}-processed.json"
